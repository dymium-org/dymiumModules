...
#       to run this test use # test_file('modules/demography/tests/testthat/test-migration.R')
# import the module
library(dymiumCore)
...
# test_that('event works', {
  ...
  n_households_before <- 
    world$AgentContainer$pop$hh$get_data() %>%
    nrow()
  ...
  for (year in years) {
    world$start_iter(year, "year") %>% 
      event_demography_migration$run(object = ., target = 100)   
  }
